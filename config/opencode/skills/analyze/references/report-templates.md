...
Load when writing up an analysis. Each section is the report shape for one
analysis type; use the one matching the scope you were asked for.

## Contents

- [Per-type report shapes](#per-type-report-shapes) — architecture, quality, performance, security, coverage, dependencies
- [Full report template](#full-report-template) — the combined write-up

## Per-type report shapes
